...
class Chatbot:
    ...
    def start_conversation(self, name):
        ...
        while True:
            print("Press 'Enter' when you are ready to respond...")
            input()  # Wait for user to press Enter to begin speaking
            user_input = self.speech_service.listen_once()
            if user_input is user_input.strip().lower() == 'quit':
    ...
    def generate_response(self, user_input):
        ...
        system_message = f"""As a training tool for the {self.position} role,imagine you are a project manager ask question that you will ask tou a fresher for the resoiurce you looking for in them. this mock interview is crafted to assist freshers in honing their interviewing skills. The exercises are specifically designed to align with what project managers look for in candidates for project roles.
